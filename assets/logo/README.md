...
- `export-group.js` — `npm run export:logo-group -- --view icon --yaw 50`. Writes a self-contained page that renders the `CocoonLogoGroup` mesh with three's WebGLRenderer, head on, turned, and the front triangle close and lit, with every prop value printed under them; a headless Chromium screenshots it to PNG. `docs/export-logo-group.md` explains the options and the camera. Output goes to `explorations/export/`.
- `export-group.test.js` — the arguments, the camera fit, the plan, the page's contents, the files written; the screenshot itself only where a Chromium is found.
