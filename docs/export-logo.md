...
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | -------------- | ------------------- |
| `--off`     | Camera offset from the front shape, in shape widths. Larger spreads the planes further sideways; the trail behind the black triangle grows with it.                                                                                                   | 1.3                         | 0.1            | > 0                 |
...
| `--gap`     | Lockup only. Clear space from the black triangle's right edge to the wordmark, in wordmark stems. Left unset, it is derived for each row by the tier rule at air2x, as the shipped lockups are, and the derived figure is printed in the row's label. | derived                     | 0.25           | ≥ 0                 |

`--size` and `--gap` are refused in the icon view.
...
- The wordmark's own instance, `wght 350 / wdth 107`, is not a parameter; it is cut once and cached. Changing it is a recut in `lockup.js`.
