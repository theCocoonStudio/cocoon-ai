...
The scheme is Stam's stable fluids as Harris describes it for the GPU: Harris, M. J., "Fast Fluid Dynamics Simulation on the GPU", _GPU Gems_ chapter 38, NVIDIA, 2004, free to read at <https://developer.nvidia.com/gpugems/gpugems/part-vi-beyond-triangles/chapter-38-fast-fluid-dynamics-simulation-gpu>. Its section 38.3 states the boundary conditions this wall implements in part: no-slip on velocity, so the velocity at the wall is zero, and pure Neumann on pressure, so each pressure boundary value is set to the value just inside, with one fragment program for both (Listing 38-5). What this hook still does differently from the chapter, zero on the rim rather than the negated neighbour, the rim's pressure computed rather than copied, the rim rewritten twice a frame rather than after every step, is the next change.
