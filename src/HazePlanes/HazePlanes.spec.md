...
markup.5: three ramps, each hazeTones toward props.9: the box ramp from props.8, the content ramp from props.10, the border ramp from props.10a. In copy k, paint.background sets `background` from the box ramp, paint.color `color` from the content ramp, paint.border `border-color` from the border ramp, as inline style on each top-level element of the copied children, merged after the element's own `style`, so they override that element's stylesheet rules (Izzy, 2026-09-06). Text children, which have no element, take `color` on the copy's wrapper instead
markup.6: elements below the top level are untouched; they take the tones only where they inherit them, currentColor for artwork and text, `inherit` for a border (fix for the old 'both', which gave box and content one tone and the content vanished)
