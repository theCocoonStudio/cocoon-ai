...
library.script: assets/logo/export-group.js, `npm run export:logo-group -- [--prop value ...]` — writes a self-contained page that renders the built pieces with three's WebGLRenderer, set up as a fiber Canvas sets it, three cells with a perspective camera and every prop value printed; a headless Chromium screenshots it to PNG when one is present; output to assets/logo/explorations/export/
